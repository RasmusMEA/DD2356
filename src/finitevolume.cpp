...
mtx applyFluxes(mtx F, const mtx &flux_F_x, const mtx &flux_F_y, double dx,
                double dt) {
  F -= (dt * dx) * flux_F_x;
  F += (dt * dx) * flux_F_x.rollDown();
  F -= (dt * dx) * flux_F_y;
  F += (dt * dx) * flux_F_y.rollRight();
...
std::array<mtx, 4> getFlux(const mtx &rho_L, const mtx &rho_R, const mtx &vx_L,
  ...
  mtx P_star = (gamma - 1) * (en_star - 0.5 * ((momx_star * momx_star) + (momy_star * momy_star)) / rho_star);

  mtx momx_rho = momx_star / rho_star;

  mtx flux_Mass = momx_star;
  mtx flux_Momx = (momx_star * momx_rho) + P_star;
  mtx flux_Momy = (momy_star * momx_rho);
  mtx flux_Energy = momx_rho * (en_star + P_star);
...
void simloop() {
  ...
  while (t < tEnd) {
    ...
    mtx rho_prime = rho - (0.5 * dt) * (vx * rho_dx + rho * vx_dx + vy * rho_dy + rho * vy_dy);
    mtx vx_prime = vx - (0.5 * dt) * (vx * vx_dx + vy * vx_dy + (1 / rho) * P_dx);
    mtx vy_prime = vy - (0.5 * dt) * (vx * vy_dx + vy * vy_dy + (1 / rho) * P_dy);
    mtx P_prime = P - (0.5 * dt) * (gamma * P * (vx_dx + vy_dy) + vx * P_dx + vy * P_dy);
